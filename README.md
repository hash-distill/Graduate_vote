...
> 本项目基于 `zkm` 和 `dcs` 两位 21 届学长的代码，由 `zhs` 和 `jqy` 进行了更新迭代，`lcp` 学长参与了问题排查与代码修复。25 年正式启用前，对前后端进行了多次内容修改和 Bug 修复，确保投票页面和逻辑的正确性。

> [!WARNING]
> 受限于前端代码的兼容性，当前系统页面仅适配 **Microsoft Edge** 浏览器。在 Chrome 等其他浏览器中打开可能出现页面错位、功能缺失等问题，请管理员和使用者注意使用 Edge 打开。
...
> [!IMPORTANT]
> 前端 JS 中的后端 API 地址默认为 `192.168.110.10`（25 年部署时的路由器 IP）。如需本地运行或更换部署环境，需全局替换为 `localhost` 或实际 IP 地址。
...
在 **Edge 浏览器**中访问 `http://<部署主机IP>` 即可使用系统。
